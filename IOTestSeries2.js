$(document).ready(
    function(){
        ...
        $('#btnAddProcess').on('click', function(){
            ...
            if(processID.val() === '' || arrivalTime.val() === '' || CPUburstTime1.val() === '' || IOburstTime.val() === '' || CPUburstTime2.val() === ''|| Priority.val() === ''){
        ...
        $('#btnCalculate').on('click', function(){
            ...
            return PrioritySchedulingPremptive();
        });


function PrioritySchedulingPremptive() {
    var completedList = [];
    var time = 0;
    var queue = [];
    ...
    while ( processList.length>0 || queue.length>0 ) {
     ...
     selectProcessForPrioritySchedulingPremptive();
     runPrioritySchedulingPremptive();
    }
    ...
    function selectProcessForPrioritySchedulingPremptive() {
        if (queue.length != 0) {
    ...
    function runPrioritySchedulingPremptive() {
        time++;
        addToQueue();
    }
    ...
    $('#tblProcessList tr').each(function(row, tr) {
        TableData[row] = {
	        ...
	        "Priority": parseInt($(tr).find('td:eq(3)').text())
        }
    });
